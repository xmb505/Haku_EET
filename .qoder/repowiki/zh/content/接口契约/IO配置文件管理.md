...
## 更新摘要
**变更内容**
- 新增 UI 配置参数 `passenger.closing_timeout_seconds`，用于控制门关闭超时保护的时间阈值
- 在 App 初始化时将该配置传递给 ActionExecutor
- 在 executor 的 CLOSE_DOOR 动作中实现超时保护逻辑
- 支持运行时通过 /reload 命令重载 ui_config.yaml 配置
...
## 引言
本文件聚焦于"Haku_EET"中 IO 配置文件管理的完整生命周期：从点位表到生成映射、从多环境 Profile 选择、到运行时加载与热更新，以及 IOClient 的读写合并策略。目标是帮助读者理解 Haku_EET 中 IO 层如何以配置为中心驱动硬件交互，并在不同场景（训练、比赛、带载重）间平滑切换。

**更新** 新增了 UI 配置参数管理，特别是门关闭超时保护机制的配置与控制。

## 项目结构
围绕 IO 配置的关键目录与文件如下：
- 顶层主配置：选择 IO Profile、IO2HTTP 连接参数、电梯运行参数等
- UI 配置：独立管理乘客交互相关参数（灯闪、关门间隔、超时保护等）
...
U["config/ui_config.yaml<br/>UI及乘客交互配置"] --> B
B --> V["ActionExecutor<br/>接收closing_timeout_seconds配置"]
V --> W["CLOSE_DOOR超时保护<br/>door_close_done检测"]
```
...
- [config/ui_config.yaml:16-19](file://config/ui_config.yaml#L16-L19)
- [core/executor.py:742-771](file://core/executor.py#L742-L771)
...
- ActionExecutor：执行器，处理各种动作（开门、关门、移动等），包含门关闭超时保护机制。
- PassengerManager：乘客管理器，处理乘客交互逻辑，管理 UI 配置参数。
- gen_io_config.py：从点位表解析并生成 io_config.yaml，屏蔽点位表格式差异，统一为系统可消费的 YAML 映射。

**更新** 新增了 ActionExecutor 的门关闭超时保护功能，通过配置项 `passenger.closing_timeout_seconds` 控制超时阈值。

章节来源
- [core/io_mapper.py:19-124](file://core/io_mapper.py#L19-L124)
- [core/io_client.py:35-104](file://core/io_client.py#L35-L104)
- [core/app.py:61-137](file://core/app.py#L61-L137)
- [core/executor.py:50-84](file://core/executor.py#L50-L84)
- [core/passenger.py:142-168](file://core/passenger.py#L142-L168)
- [tools/gen_io_config.py:173-195](file://tools/gen_io_config.py#L173-L195)
...
- **新增** UI 配置参数通过 PassengerManager 管理，包括门关闭超时保护时间
...
participant UICfg as "ui_config.yaml"
...
App->>UICfg : 加载passenger.closing_timeout_seconds
App->>Executor : 传递closing_timeout_seconds配置
Note over ClientR,PLC : WS订阅bitmap/增量事件
Note over ClientW,PLC : 定时批量POST写DB区
Note over Executor,PLC : CLOSE_DOOR超时保护检查
```

图表来源
- [core/app.py:61-137](file://core/app.py#L61-L137)
- [core/io_mapper.py:33-76](file://core/io_mapper.py#L33-76)
...
### ActionExecutor：动作执行与超时保护
- 职责
  - 执行各种电梯动作（初始化、移动、开关门等）
  - 处理传感器信号，推进状态机
  - **新增** 实现门关闭超时保护机制
- 关键流程
  - run_loop(): 主循环，从队列取动作并执行
  - _execute_close_door(): 执行关门动作，包含超时保护逻辑
  - on_io_event(): 处理 IO 事件，更新状态
- **新增** 超时保护机制
  - 在 CLOSE_DOOR 动作中，如果超过配置的 `closing_timeout_seconds` 未收到 door_close_done 信号
  - 主动查询 IO 状态：若 door_close_done=1 则正常完成；若仍为 0 则亮故障灯并停止调度
  - 防止因 PLC 边沿检测漏掉导致 door_close_done 信号永久不触发的问题

```mermaid
flowchart TD
Start(["开始CLOSE_DOOR"]) --> CheckTimeout{"closing_timeout_seconds > 0?"}
CheckTimeout --> |否| WaitDone["等待door_close_done"]
CheckTimeout --> |是| StartTimer["启动超时计时器"]
StartTimer --> WaitDone
WaitDone --> Timeout{"是否超时?"}
Timeout --> |否| Complete["正常完成关门"]
Timeout --> |是| CheckIO["主动查询door_close_done IO状态"]
CheckIO --> IOState{"IO状态是否为1?"}
IOState --> |是| NormalComplete["正常完成(IO已确认关好)"]
IOState --> |否| Fault["亮故障灯+停止调度"]
...
- 关键点
  ...
  - **新增** 从 ui_config.yaml 加载 passenger.closing_timeout_seconds 并传递给 ActionExecutor
...
App --> PassengerManager : "乘客交互管理"
App --> ActionExecutor : "传递超时配置"
```

图表来源
- [core/app.py:61-137](file://core/app.py#L61-L137)
- [core/app.py:165-173](file://core/app.py#L165-L173)

章节来源
- [core/app.py:61-137](file://core/app.py#L61-L137)
- [core/app.py:165-173](file://core/app.py#L165-L173)

### PassengerManager：乘客交互管理
- 职责
  - 管理乘客交互逻辑，处理外召、内召按钮事件
  - 管理 UI 配置参数，支持运行时重载
  - 协调各轿厢的乘客服务流程
- 关键流程
  - _reload_ui_config(): 重新加载 ui_config.yaml 配置
  - _door_close_delay(): 获取关门延时配置
  - _human_presence_off_delay(): 获取无人确认延时配置
  - _flash_interval(): 获取指示灯闪烁间隔配置
- **新增** 支持动态配置重载，无需重启程序即可应用新的 UI 参数

章节来源
- [core/passenger.py:142-168](file://core/passenger.py#L142-L168)
- [core/passenger.py:202-226](file://core/passenger.py#L202-L226)
...
- 配置依赖
  ...
  - ui_config.yaml 管理乘客交互相关参数（包括门关闭超时保护）
...
- 运行时依赖
  ...
  - ActionExecutor 依赖配置项 controlling 超时保护行为

```mermaid
graph LR
CFG["config/config.yaml"] --> APP["core/app.py"]
UICFG["config/ui_config.yaml"] --> PM["core/passenger.py"]
UICFG --> EXE["core/executor.py"]
...
- **新增** 超时保护性能影响
  - closing_timeout_seconds 配置影响关门操作的等待时间
  - 超时后主动查询 IO 状态会增加少量网络开销，但能及时发现硬件故障
  - 建议设置为合理值（默认10秒），平衡响应速度与故障检测及时性
...
- **新增** 门关闭超时保护相关问题
  - 现象：关门操作长时间无响应，最终触发故障
  - 原因：door_close_done 信号未正确触发或配置超时时间过短
  - 处理：检查 door_close_done 信号配置，调整 closing_timeout_seconds 参数
  - 调试：查看日志中的 "[exec] carX CLOSE_DOOR timeout" 相关信息
...
## 结论
Haku_EET 的 IO 配置体系以"配置即真相"为核心：点位表作为唯一事实源，经工具生成标准化映射；多环境 Profile 实现场景化切换；运行时通过 IOMapper 与 IOClient 解耦业务逻辑与硬件细节，并以写合并与事件过滤保障性能与稳定性。**新增的 UI 配置参数管理进一步增强了系统的灵活性和可靠性**，特别是门关闭超时保护机制能够有效应对硬件信号异常的情况。遵循此设计，可在不同部署环境下快速适配，同时保持代码稳定与可维护性。
...
- **新增** UI 配置 key（位于 ui_config.yaml）
  - passenger.queue_mode：队列模式（'discard' | 'keep'）
  - passenger.door_close_delay_ms：关门延时（毫秒）
  - **passenger.closing_timeout_seconds：门关闭超时保护时间（秒）**
  - passenger.human_presence_off_delay：无人确认延时（秒）
  - passenger.flash_interval_ms：外召指示灯闪烁间隔（毫秒）
- IO 映射结构
...
章节来源
- [config/config.yaml:1-20](file://config/config.yaml#L1-L20)
- [config/ui_config.yaml:6-28](file://config/ui_config.yaml#L6-L28)
