...
## 更新摘要
**变更内容**   
- 新增重量检测三态机管理系统，实现状态0(正常)/状态1(临界)/状态2(超载)的完整重量监控逻辑
- 集成ADC转换和自动门重开功能，支持实时重量监测和安全保护
- 完善硬件控制模块的慢刹车系统文档，详细说明低速运行时的制动控制机制
...
- 实现机制
  ...
  - **后台轮询**：独立asyncio任务持续读取重量数据，间隔可配置(weight_poll_interval_ms)。
- 安全保护机制
...
- 配置管理
  ...
  - **应急轮询**：超载状态下缩短轮询间隔(默认200ms)快速检测恢复。
- 调试监控
...
- [core/executor.py:1161-1204](file://core/executor.py#L1161-L1204)
- [core/player.py:87-92](file://core/player.py#L87-L92)

**章节来源**
- [core/weight_manager.py:1-103](file://core/weight_manager.py#L1-L103)
- [core/executor.py:1133-1204](file://core/executor.py#L1133-L1204)
- [core/app.py:207-222](file://core/app.py#L207-L222)
- [core/player.py:87-92](file://core/player.py#L87-L92)
- [config/config.yaml:62-74](file://config/config.yaml#L62-L74)
...
**更新** 新增了重量检测系统的硬件验证建议，确保ADC转换精度和三态机逻辑的正确性。
