...
## 更新摘要
**所做更改**
- 更新了重量轮询机制架构，从连续后台轮询重构为按需轮询
- 修改了重量查询触发时机，仅在关键操作时进行IO读取
- 增强了每车重量配置的灵活性
- 优化了系统性能，减少不必要的I/O操作
...
**更新** 重量检测系统已从连续后台轮询重构为按需轮询机制，显著提升了系统性能和资源利用率。
...
**图表来源**
- [core/app.py:1-1794](file://core/app.py#L1-L1794)
- [core/console.py:690-750](file://core/console.py#L690-L750)
- [core/executor.py:1330-1418](file://core/executor.py#L1330-L1418)
...
**章节来源**
- [core/app.py:61-1794](file://core/app.py#L61-L1794)
- [core/weight_manager.py:1-106](file://core/weight_manager.py#L1-L106)
- [core/io_client.py:1-32](file://core/io_client.py#L1-L32)
- [core/console.py:690-750](file://core/console.py#L690-L750)
- [example_web/dashboard.html:589-619](file://example_web/dashboard.html#L589-L619)
...
**更新** 重量检测架构已重构为按需轮询模式，减少了不必要的I/O操作。
...
- [core/executor.py:1359-1362](file://core/executor.py#L1359-L1362)
...
**更新** 重量检测系统已从连续后台轮询重构为按需轮询机制，显著提升了系统性能。
...
Check --> |是| Config["加载 per_car_weight 配置"]
Check --> |否| End(["结束"])
Config --> Ready["等待触发事件"]
subgraph "触发事件"
DoorClose["关门开始"]
...
- [core/weight_manager.py:29-50](file://core/weight_manager.py#L29-L50)
- [core/console.py:687-694](file://core/console.py#L687-L694)
- [core/executor.py:1333-1342](file://core/executor.py#L1333-L1342)

#### 重量查询触发点

**关门流程中的重量检查：**
1. `on_close_door_starting`：关门开始前立即查询重量
...
- **更新** 重量读取由 weight_manager 在关键时机按需触发，而非后台持续轮询。
...
- **更新** WeightManager 作为独立模块，专门处理重量相关的业务逻辑。
...
- [example_web/control.html:496-518](file://example_web/control.html#L496-L518)
- [core/app.py:1732-1745](file://core/app.py#L1732-L1745)
- [config/config.yaml:70-78](file://config/config.yaml#L70-L78)
...
**更新** 按需轮询机制的成功实施为后续的功能扩展奠定了坚实基础，系统整体稳定性和性能得到显著提升。
...
- [core/executor.py:1363-1418](file://core/executor.py#L1363-L1418)
...
- [core/executor.py:1383-1393](file://core/executor.py#L1383-L1393)