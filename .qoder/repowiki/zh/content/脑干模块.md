...
- IOMapper：加载 io_config.yaml，维护 DB/I 地址映射与反向索引
...
MAPPER["IOMapper<br/>DB↔I 映射/反向索引"]
...
- IOMapper
  - 职责：加载配置、提供逻辑名→DB 地址查表、DB↔I 换算、I 地址→(car_id, signal) 反查
  - 关键接口：addr_output/addr_input、db_to_i/i_to_db、lookup_signal_by_i、lookup_all_i_addresses
- VirtualPLC
...
### IOMapper：IO 映射表与地址换算
- 配置加载
  - 从 io_config.yaml 读取 db_to_i_offset、per_car/hall 的输入输出点位
  - 构建正向映射（car_id+signal → DB 地址）、反向索引（DB 地址 → car_id+signal）、I 地址 → (car_id, signal) 索引
- 地址换算
  - db_to_i：根据偏移将 DBX.byte.bit 转换为 I.byte'.bit
  - i_to_db：反向转换，输入默认来自固定 DB 块号
- 使用场景
  - 上层通过 addr_output/addr_input 获取 DB 地址
...
章节来源
- [core/io_mapper.py:19-105](file://core/io_mapper.py#L19-L105)
- [config/io_config.yaml:1-502](file://config/io_config.yaml#L1-502)
...
- 与 IOClient 的交互
  - 通过 mapper.addr_input/db_to_i 计算 I 地址
...
alt 电机运行且方向确定
VPLC->>VPLC : 计算新虚拟位置
VPLC->>MAP : addr_input(level_up/down) + db_to_i
VPLC->>IO : simulate_input(I_addr, 1)
VPLC->>IO : 延时后 simulate_input(I_addr, 0)
else 到达限位
VPLC->>MAP : addr_input(limit_1/2) + db_to_i
VPLC->>IO : simulate_input(I_addr, 1)
VPLC->>IO : 延时后 simulate_input(I_addr, 0)
end
end
...
- IOMapper 依赖 yaml 解析配置文件，正则匹配地址格式
- VirtualPLC 依赖 IOClient 的 simulate_input 与 get_output，依赖 IOMapper 的地址换算
...
- 虚拟 PLC 未驱动输入
  ...
  - 排查：确认 io.simulate=True；检查 VirtualPLC.start 是否被调用；核对 addr_input/db_to_i 结果
...
## 结论
脑干模块通过 IOClient 实现了稳定的 WS 订阅与 HTTP 写合并，结合 IOMapper 的映射表与地址换算，为上层提供了统一的 IO 抽象；在 simulate 模式下，VirtualPLC 反向驱动输入事件，形成完整的端到端仿真闭环。该设计兼顾了实时性与可靠性，适合多轿厢并发场景与调试验证需求。